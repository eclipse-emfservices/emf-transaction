...
public class PrivilegedRunnableTest extends AbstractTest {
	static final IStatus TEST_STATUS = new Status(IStatus.OK, TestsPlugin.instance.getBundle().getSymbolicName(), 0,
			"OK", null);
	...
	/**
	 * Tests that a thread that has borrowed a transaction via a privileged runnable
	 * can lend it along to yet another thread.
	 */
	@Test
	...
	/**
	 * Tests the assertion that a transaction be active when executing a privileged
	 * runnable.
	 */
	@Test
	...
	/**
	 * Ensure that the catching of run-time exceptions in privileged runnables does
	 * not cause an overriding <code>IllegalArgumentException</code> on initializing
	 * a <code>Status</code> with a <code>null</code> message from the exception.
	 * The privileged runnable must have a status when finished, whether
	 * successfully or not.
	 */
	@Test
