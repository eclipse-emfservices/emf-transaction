...
public class UndoRedoTest extends AbstractTest {
	...
	public void test_triggerCommands_nonRecording() {
		...
		// check that the library exists and was correctly configured with a default
		// book
		startReading();
	...
	/**
	 * Tests that the changes made by trigger commands can be undone and redone when
	 * the original command is a recording command.
	 */
	@Test
	 ...
	 * editing domain (which would mean that our strategy of using a change recorder
	 * for undo/redo is flawed).
	 */
	@Test
	public void test_undoResourceUnload() {
		...
		assertNotNull(book);
		Writer author = (Writer) find("root/level1/Level1 Writer");
...
//		assertFalse(author.eIsProxy());// TODO: Should un-proxify on attachment
	...
	/**
	 * Tests that undoing the load of a resource has no effect (as it does not alter
	 * that abstract state of the model/resource set).
	 */
	@Test
	...
	/**
	 * Tests that trigger commands are not undone and redone multiple times when
	 * triggers are propagated to a parent transaction.
	 */
	@Test
	...
	public void test_nonredoableTriggerCommand_138287() {
		...
		cmd.chain(AddCommand.create(domain, root, EXTLibraryPackage.Literals.LIBRARY__BRANCHES, newLibrary));
	...
	public void test_undoRedoOptionsNotSharedbyDomains_bug207986() {
		InternalTransactionalEditingDomain otherDomain = (InternalTransactionalEditingDomain) TransactionalEditingDomain.Factory.INSTANCE
				.createEditingDomain();

		final String BOGUS_OPTION = "**bogus_option**";

		otherDomain.getUndoRedoOptions().put(BOGUS_OPTION, Boolean.TRUE);

		assertFalse(((InternalTransactionalEditingDomain) domain).getUndoRedoOptions().containsKey(BOGUS_OPTION));
	}
	 ...
	 * executing recording-commands on the command-stack.
	 */
	@Test
	...
	/**
	 * Tests that recording-commands used as triggers are not undone twice when
	 * executing recording-commands that are nested in compound commands that are
	 * executing on the command-stack.
	 */
	@Test
